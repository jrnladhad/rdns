...
impl FQDN {
    fn get_label(decoder: &mut BinReader) -> FqdnResult<String> {
        ...
        let label = decoder.read_n_bytes(label_len as u16)
    ...
    fn recursively_create_name(decoder: &mut BinReader, jump_count: u16) -> FqdnResult<Vec<String>> {
        ...
        loop {
            parsing_fsm = match parsing_fsm {
                FqdnParsingFSM::PtrOrLen => Self::get_parsing_state(decoder)?,
                ...
                FqdnParsingFSM::Pointer => {
                    ...
                    let mut parsed_labels = Self::recursively_create_name(&mut cloned_decoder, jump_count + 1)?;
